...
def get_rag_chain(model="gemini-1.5-flash"):
    ...
    try:
        ...
        contextualize_q_prompt = ChatPromptTemplate.from_messages([
            ("human", """Given the chat history and the latest user question, reformulate the question into a clear, standalone query that captures the core intent. If the question is vague (e.g., "yo" or "hey"), interpret it as a request for general academic guidance at ASTU, focusing on common student needs like course prerequisites, registration, or academic standing. Keep it short and ASTU-specific. Do not answer or add extra context and never explain the source like 'Based on the provided text','Based on the provided document' etc.
